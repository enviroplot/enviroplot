...
router.post('/', upload.fields([
  ...
  try {
    ...
    if (!req.files || !projectId) return res.status(400).json({ error: 'Missing files or project ID' });

    const result = await parseEsdatBundle({
      sample: req.files['sample'][0].buffer,
      chemistry: req.files['chemistry'][0].buffer,
      header: req.files['header']?.[0]?.buffer
    });
